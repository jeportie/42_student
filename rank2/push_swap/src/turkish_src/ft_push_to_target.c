...
/*   Updated: 2024/07/18 13:39:07 by jeportie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (cheapest_node && cheapest_node != stack_a->begin)
	{
	...
	while (cheapest_node->target && cheapest_node->target != stack_b->begin)
	{
...
void	ft_execute_combined_moves(t_dclst *stack_a, t_dclst *stack_b,
		t_dcnode *cheapest_node)
{
	while (cheapest_node != stack_a->begin && cheapest_node->target
		!= stack_b->begin)
	{
		if (cheapest_node->above_median)
			rr(stack_a, stack_b);
